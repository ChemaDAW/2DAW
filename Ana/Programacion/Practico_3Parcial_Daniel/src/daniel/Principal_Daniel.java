...
public class Principal_Daniel {
    ...
    public static void altaCaballo() {
        ...
        try {
            ...
            System.out.println(VERDE + "Se ha dado de alta el caballo [" + AZUL
                    + (cuadras.get(cuadra).getResidentes().size() - 1) + VERDE + "]:\n" + cuadras.get(cuadra)
                            .getResidentes().get(cuadras.get(cuadra).getResidentes().size() - 1).toString());
    ...
    public static void altaCarrera() {
        ...
        System.out.println("Indica el ganador de la carrera (0-"
                + (carreras.get(carreras.size() - 1).getParticipantes().size() - 1) + "):");
        carreras.get(carreras.size() - 1)
                .setGanador(carreras.get(carreras.size() - 1).getParticipantes().get(pedirNum()));
        ...
        do {
            ...
            try {
                ...
                if (cuadras.get(cuadra).getResidentes().isEmpty()) {
                    ...
                    System.out.println("Indica el caballo participante (0-"
                            + (cuadras.get(cuadra).getResidentes().size() - 1) + "):");
                    participante = pedirNum();
                    c.insertarParticipante(cuadras.get(cuadra).getResidente(participante));
                    System.out.println(VERDE + "¡Se ha añadido el caballo [" + AZUL + participante + VERDE
                            + "] de la cuadra [" + AMARILLO + cuadra + VERDE + "]" + RESET);
                }
            } catch (IndexOutOfBoundsException e) {
                System.out.println(ROJO + "¡No existe esa posición!" + RESET);
            }
            System.out.println(
                    "Si quieres añadir otro partipicante escribe cualquier cosa, de lo contrario pulsa enter sin indicar nada.");
    ...
    public static void consultarCuadraGanadora() {
        // No me da tiempo, habría que comprobar todas las carreras la cuadra de los
        // caballos ganadores y el mostra la de mayor resultado
    }
    ...
    public static void elminarCaballo() {
        ...
        try {
            System.out.println(
                    "Indica el caballo a eliminar (0-" + (cuadras.get(cuadra).getResidentes().size() - 1) + "):");
